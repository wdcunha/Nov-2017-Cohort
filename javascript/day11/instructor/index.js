...
const ArrayExtras = {
  last: function (arr) { // ð Traditional way to write a method
    return arr[arr.length - 1];
  },
  first: first,
  take (qty, arr) { // ð Using short-hand syntax (only works for methods)
    return arr.slice(0, qty);
  },
  drop (qty, arr) { // ð Using short-hand syntax (only works for methods)
...
console.log("In script scope:", this); // ð returns the global `window` object
...
// ð `this` is the object that owns a method. In this context, `touchThis` is
...
// ð the function `can.touchThis` has been assigned to a variable therefore
...
class Doggo {
  ...
  // } ---->the code bellow is almost the same, but refectored to a be used here
  bark () {
    return `${this.name} says ${['bork', 'bark', 'woof'][Math.floor(Math.random() * 3)]}`;
  }
}
...
// Doggo.prototype.bark = function () {
//   return ['bork', 'bark', 'woof'][Math.floor(Math.random() * 3)];
// }
...
function DoggoFighter (name, age, specialAbility) {
  ...
  // ð Using Object.assign to mutate `this`
  ...
  // ð Object property shorthand syntax
...
//it's not the same, but changed for another result
// class DoggoFighter extends Doggo {
//   // Inside a JavaScript class, you can only write methods
//   // inside the braces.
//   constructor (name, age, specialAbility) {
//     super(name, age); //important to call here, before to change it bellow or an error will be given
//     // ð super(...) is special function usable inside a constructor
